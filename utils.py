...
# 生成完整的城市选项列表（格式：xx省xx市）
def generate_city_options():
    """生成完整的城市选项列表"""
    options = []
    for province, cities in CHINA_CITIES.items():
        for city in cities:
            options.append((f"{province}{city}", f"{province}{city}"))
    return options

# 生成省份选项列表
def generate_province_options():
    """生成省份选项列表"""
    options = []
    for province in CHINA_CITIES.keys():
        options.append((province, province))
    return options

# 根据省份获取城市列表
def get_cities_by_province(province):
    """根据省份获取城市列表"""
    return CHINA_CITIES.get(province, [])